...
from features import (fit_question_vectoriser, normalise_question, read_split,
                      to_features)
from paths import PRIVATE_DATA, assert_inside_project
from split import (assign_groups_to_splits, check_every_chapter_has_test_rows,
...
DEFAULT_CONDITIONS = {"class_weights": True, "max_tokens": 512}
...
class Run:
    ...
    method: str = METHOD
    features: int = None
    conditions: dict = field(default_factory=lambda: dict(DEFAULT_CONDITIONS))
...
def resplit_tfidf(rows, seed):
    ...
    """
    split_rows = assign_groups_to_splits(rows, seed)
    check_no_group_straddles_splits(split_rows)
    check_rows_conserved(rows, split_rows)
    check_every_chapter_has_test_rows(split_rows)
...
def record_path(run):
    return RUNS_DIR / f"{run.approach}-seed{run.seed}-{run.test_hash}.json"


def record_run(run):
    """Write one run to the ledger, replacing an earlier run of the same seed."""
    path = assert_inside_project(record_path(run))
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(asdict(run), indent=1), encoding="utf-8")
    return path


def load_runs(approach, test_hash=None, conditions=DEFAULT_CONDITIONS):
    """Every recorded run of one approach against the current test split.

    Records made against another split, or under non-default conditions, are
    left out without comment; the caller sees only runs that can be compared.
    """
    if test_hash is None:
        test_hash = split_hash()
    runs = []
    for path in sorted(RUNS_DIR.glob(f"{approach}-seed*-*.json")):
        record = json.loads(path.read_text(encoding="utf-8"))
        if record.get("test_hash") != test_hash:
            continue
        if conditions is not None and record.get("conditions") != conditions:
            continue
